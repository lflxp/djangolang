...
// 编辑表单字段
var editColumns = map[orm.FieldType]string{
...
var editFuncMap = map[orm.FieldType]func(string, []string, map[string][]byte) string{
	orm.Password: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1), "$TYPE", "password", -1), "$VALUE", string(data[tmp[2]]), -1)
	},
	orm.StringField: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1), "$TYPE", "text", -1), "$VALUE", string(data[tmp[2]]), -1)
	},
	orm.IntField: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1), "$TYPE", "number", -1)
	},
	orm.Int16Field: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1), "$TYPE", "number", -1)
	},
	orm.Int64Field: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1), "$TYPE", "number", -1)
	},
	orm.Textarea: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "$NAME", tmp[2], -1), "$LABELS", tmp[0], -1)
	},
	orm.Radio: func(text string, tmp []string, data map[string][]byte) string {
		tmp_radio := ""
		for _, r := range strings.Split(tmp[3], ",") {
			tmp_ro := strings.Split(r, "|")
			if tmp_ro[1] == string(data[tmp[2]]) {
	...
	orm.Select: func(text string, tmp []string, data map[string][]byte) string {
		tmp_select := ""
		for _, s := range strings.Split(tmp[3], ",") {
			tmp_se := strings.Split(s, "|")
			if tmp_se[1] == string(data[tmp[2]]) {
	...
	orm.MultiSelect: func(text string, tmp []string, data map[string][]byte) string {
		tmp_multiselect := ""
		for _, s := range strings.Split(tmp[3], ",") {
			tmp_se := strings.Split(s, "|")
			if strings.Contains(string(data[tmp[2]]), tmp_se[1]) {
	...
	orm.File: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "$NAME", tmp[2]+"\" id='"+tmp[2]+"'", -1), "$LABELS", tmp[0], -1), "$TYPE", "file", -1)
	},
	orm.Time: func(text string, tmp []string, data map[string][]byte) string {
		return strings.Replace(strings.Replace(strings.Replace(strings.Replace(text, "$VALUE", string(data[tmp[2]]), -1), "-timepicker1", fmt.Sprintf("%d", time.Now().Nanosecond()), -1), "$LABELS", tmp[0], -1), "$NAME", tmp[2], -1)
	},
	orm.OneToOne: func(text string, tmp []string, data map[string][]byte) string {
		tmp_o2o_string := ""
		...
		for _, x := range resultSql {
			var t_s string
			if strings.Contains(string(data[tmp[2]]), string(x[showCol[0]])) {
	...
	orm.OneToMany: func(text string, tmp []string, data map[string][]byte) string {
		tmp_o2o_string := ""
		tmp_o2o := strings.Split(tmp[3], "|")
		// log.Error("tmp_o2o", tmp[3])
		sql := fmt.Sprintf("select * from %s", tmp_o2o[0])
