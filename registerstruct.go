...
func RigsterStruct(data interface{}) map[string]string {
	...
	for i := 0; i < v.NumField(); i++ {
		// strings.ToLower(v.Type().Field(i).Name) != "id" && v.Type().Field(i).Tag.Get("name") != "id" &&
		if v.Type().Field(i).Tag.Get("xorm") != "-" && strings.ToLower(v.Type().Field(i).Name) != "create" && strings.ToLower(v.Type().Field(i).Name) != "update" {
			//利用反射获取structTag
